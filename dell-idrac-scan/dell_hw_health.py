...
def send_report():
    ...
    f = open('hw_inventory.txt', 'r')
    ntfy_msg = f.read()
    f.close()

    requests.post(args['n'], data=ntfy_msg.encode(encoding='utf-8'))
    os.system('rm hw_inventory.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-cl', help='Client Name', required=True)
    parser.add_argument('-n', help='ntfy server name', required=True)
    ...
    f = open('hw_inventory.txt', 'w')
    f.write(f'Dell Health Check for Client {client_name} - ')
    f.close()
