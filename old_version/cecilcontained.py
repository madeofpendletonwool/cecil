...
parser.add_argument("--client_name", help="Which Client are we running for")
...
parser.add_argument("--docker_monitor_active", help="true or false - Should we run the docker monitor?", required=True)
parser.add_argument("--linux_health_active", help="true or false - Should we run the linux health scan?", required=True)
parser.add_argument("--dynamic_ip_updater", help="true or false. Should be run the dynamic IP Scanner", required=True)
...
parser.add_argument("--dell_scan_cron", help="cron timer of how often the dyanmic IP updater should run")
parser.add_argument("--dell_scan_active", help="true or false - Should we run the docker monitor?", required=True)
...
# Check if the entry already exists in the crontab
entry_exists_dell = os.system("crontab -l | grep '/opt/cecil/monitor-docker/cronstart.py'")

#Check if Docker Monitor should run and setup
if args_passed['dell_scan_active'] == 'true' and entry_exists_dell != 0:
    setup_dell_scan = f"crontab -l 2>/dev/null; echo '{args_passed['dell_scan_cron']} /usr/bin/python3 /opt/cecil/dell-idrac-scan/dell_hw_health.py -ip $IDRAC_IP -u $IDRAC_USER -p $IDRAC_PW -n $MONITOR_URL -cl $CLIENT -a' | crontab -"
    os.system(setup_dell_scan)
